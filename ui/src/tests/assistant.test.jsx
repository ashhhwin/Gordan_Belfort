    ...
    const tools = {
      get_net_worth_summary: (holdings) => {
        const assets = holdings.reduce(
          (s, h) => s + (h.cmp > 0 ? h.cmp * h.qty : 0),
          0,
        );
        const liabilities = holdings.reduce(
          (s, h) => s + (h.cmp < 0 ? Math.abs(h.cmp * h.qty) : 0),
          0,
        );
  ...
  it("48. AI Tooling (Portfolio): get_portfolio_details returns JSON array of symbols", () => {
    const tools = {
      get_portfolio_details: (holdings) => {
        return JSON.stringify(
          holdings.map((h) => ({ symbol: h.symbol, val: h.cmp * h.qty })),
        );
      },
    };

    const res = JSON.parse(
      tools.get_portfolio_details([{ symbol: "AAPL", cmp: 150, qty: 10 }]),
    );
