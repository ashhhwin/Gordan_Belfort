...
describe('Portfolio Calculations & Math', () => {
  ...
  const sampleHoldings = [
    { assetClass: 'IND_EQUITY', qty: 10, avgBuy: 100, cmp: 150, buyDate: new Date(Date.now() - 400 * 24 * 60 * 60 * 1000).toISOString() },
    { assetClass: 'CREDIT_CARD', qty: 1, avgBuy: -500, cmp: -500 },
    { assetClass: 'CRYPTO', qty: 2, avgBuy: 1000, cmp: 1200, buyDate: new Date().toISOString() },
  ];
  ...
  it('30. Liability Math: totalLiabilities correctly sums absolute values', () => {
    const totalLiabilities = Math.abs(sampleHoldings
      .filter(h => h.assetClass === 'CREDIT_CARD')
      .reduce((s, h) => s + h.cmp * h.qty, 0));
  ...
  it('32. PnL Math: totalPnl accurately calculates current value minus invested value', () => {
    const tradable = sampleHoldings.filter(h => ['IND_EQUITY', 'CRYPTO'].includes(h.assetClass));
  ...
  it('36. Tax Math: calculateTaxMetrics applies 30% flat tax correctly to CRYPTO assets', () => {
    const cryptoHolding = { assetClass: 'CRYPTO', qty: 2, avgBuy: 1000, cmp: 1200, buyDate: new Date().toISOString() };
