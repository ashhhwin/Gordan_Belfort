...
function MockHoldingsModal({ initialData, onSubmit }) {
  const isNonTradable = ["BANK", "CREDIT_CARD", "EPF"].includes(
  ...
  return (
    ...
    >
      <input
        data-testid="assetClass"
        defaultValue={initialData?.assetClass || "IND_EQUITY"}
      />
      <input
        data-testid="qty"
        defaultValue={isNonTradable ? 1 : initialData?.qty || ""}
      />
      <input
        data-testid="symbol"
        defaultValue={
          isNonTradable ? initialData?.assetClass : initialData?.symbol || ""
        }
      />
...
describe("UI Components (Holdings Modal & Tables)", () => {
  ...
  it("37. HoldingsModal (Edit): Pre-fills form data correctly", () => {
    render(
      <MockHoldingsModal
        mode="edit"
        initialData={{ assetClass: "IND_EQUITY", qty: 50, symbol: "RELIANCE" }}
        onSubmit={() => {}}
      />,
    );
  ...
  it("38. HoldingsModal (Add): Sets default empty state correctly", () => {
    render(
      <MockHoldingsModal mode="add" initialData={{}} onSubmit={() => {}} />,
    );
  ...
  it("41. HoldingsModal (Submit): Triggers addHolding method on submit", () => {
    ...
    render(
      <MockHoldingsModal mode="add" initialData={{}} onSubmit={handleSubmit} />,
    );
