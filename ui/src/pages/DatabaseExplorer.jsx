        ...
        {data.columns.map((col) => {
          ...
          return (
            ...
            >
              ...
              >
                {isPK ? (
                  <Key size={12} color="var(--accent-yellow)" />
                ) : (
                  <span style={{ width: 12 }} />
                )}
                {col.column_name}
...
const getLayoutedElements = async (nodes, edges) => {
  const elkNodes = nodes.map((n) => ({
    ...
    ports: n.data.columns
      .map((col) => [
        {
          id: `${n.id}-${col.column_name}-target`,
          properties: { side: "WEST" },
        },
        {
          id: `${n.id}-${col.column_name}-source`,
...
export default function DatabaseExplorer() {
  ...
  useEffect(() => {
    ...
    setMetaLoading(true);
    getDatabaseMetadata(selectedDb)
      .then((data) => {
        setMeta(data);
                ...
                {meta.databases.map((db) => (
                  <div
                    ...
                    style={{
                      ...
                      borderRadius: "4px",
                      background:
                        selectedDb === db ? "var(--surface-2)" : "transparent",
                    }}
                  ...
                  <div key={table} style={{ marginBottom: "4px" }}>
                    <div
                      style={{
                        ...
                        cursor: "pointer",
                        borderRadius: "4px",
                        background: expandedTables[table]
                          ? "var(--surface-2)"
                          : "transparent",
                      }}
