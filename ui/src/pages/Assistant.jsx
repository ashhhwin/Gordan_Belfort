...
const SUGGESTIONS = [
  { text: '📊 Backtest SMA crossover on RELIANCE', msg: 'Run a backtest on RELIANCE using the SMA crossover strategy with 20/50 day windows' },
  { text: '🎲 Monte Carlo forecast for TCS', msg: 'Run a Monte Carlo simulation on TCS with 1000 paths for the next 90 days' },
  { text: '💰 Show my portfolio overview', msg: 'Show me my complete portfolio with holdings, P&L, and asset allocation' },
  { text: '🧠 Markov chain analysis on NIFTY', msg: 'Run a Markov chain regime analysis on NIFTY to identify market regimes and transition probabilities' },
];
...
function ToolCard({ name, args, result, status }) {
  ...
  const Icon = status === 'running' ? Loader2 : status === 'success' ? Check : AlertCircle;

  return (
    <div>
      <div className={`tool-card ${status}`} onClick={() => result && setShowResult(!showResult)}>
    ...
    <div className="ai-markdown">
      {parts.map((part, index) => {
        if (part.startsWith('[[IMAGE_BASE64:') && part.endsWith(']]')) {
          ...
          return (
            <div key={index} style={{ margin: '14px 0', borderRadius: '10px', overflow: 'hidden', border: '1px solid rgba(255,255,255,0.06)' }}>
              <img src={`data:image/png;base64,${base64}`} alt="Generated Chart" style={{ width: '100%', display: 'block' }} />
            </div>
          );
        }
        ...
        return (
          <ReactMarkdown
            ...
            components={{
              ...
              img: ({ src, alt }) => (
                <div style={{ margin: '14px 0', borderRadius: '10px', overflow: 'hidden', border: '1px solid rgba(255,255,255,0.06)' }}>
...
export default function Assistant() {
  ...
  const createNewConversation = useCallback(async () => {
    try {
      const resp = await fetch(`${API_BASE}/conversations`, {
        ...
        body: JSON.stringify({ title: 'New Conversation', persona: selectedPersona })
      });
      if (resp.ok) {
        const data = await resp.json();
  ...
  const handleSend = async (overrideMsg) => {
    ...
    try {
      ...
      console.error(err);
      toast.error('Connection failed. Is the FastAPI backend running on :8000?');
    } finally {
      setIsTyping(false);
      setIsThinking(false);
      setMessages(prev => {
        ...
        <div className="chat-main">
          ...
          <div className="chat-header">
            <div className="chat-header-left">
              ...
              <div>
                <div style={{ fontSize: '16px', fontWeight: 600 }}>Gordan Belfort</div>
            ...
            <div className="chat-header-right">
              ...
              >
                ...
                {personas.length === 0 && <option value="gordan_belfort">Gordan Belfort</option>}
              </select>
            ...
            {!hasMessages && (
              <div className="chat-empty-state">
                ...
                <div className="chat-empty-subtitle">
                  Your elite quantitative finance copilot. Backtesting, Monte Carlo simulations, 
                  ML models, portfolio analysis, and more — all powered by 16+ specialized tools.
                </div>
          ...
          <div className="chat-input-area">
            <div className="chat-input-container">
              <textarea
                ...
                placeholder="Ask me to run a backtest, analyze risk, forecast prices, or query your portfolio..."
                rows={1}
              />
              <button
