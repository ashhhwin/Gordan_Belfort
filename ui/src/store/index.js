...
import {
  ...
  getPortfolioHistory, getSyncStatus, getSyncLogs, getCronStatus
...
export const useStore = create((set, get) => ({
  ...
  async initApp() {
    try {
      ...
      toast.error("Database Connection Failed. Please ensure Postgres is running.");
    }
  },
  ...
  async registerTouchId() {
    ...
    if (window.require) {
      try {
        const { ipcRenderer } = window.require('electron');
        await ipcRenderer.invoke('prompt-touch-id', 'Enroll Touch ID for Gordan Belfort');
        
        // If it didn't throw, authentication was successful
        await setUserWebAuthnCred(activeUser.id, 'electron-native-touch-id');
        const { users } = get();
        const updatedUsers = users.map(u => u.id === activeUser.id ? { ...u, webauthn_cred_id: 'electron-native-touch-id' } : u);
        const updatedUser = updatedUsers.find(u => u.id === activeUser.id);
        
        set({ users: updatedUsers, activeUser: updatedUser, isAuthenticated: true, isFirstRun: false, authError: null });
    ...
    try {
      ...
      const userIdBuf = await crypto.subtle.digest('SHA-256', new TextEncoder().encode(activeUser.id));
      ...
      const { users } = get();
      const updatedUsers = users.map(u => u.id === activeUser.id ? { ...u, webauthn_cred_id: credId } : u);
      const updatedUser = updatedUsers.find(u => u.id === activeUser.id);
      ...
      const msg = err.name === 'NotAllowedError' ? 'Touch ID was cancelled.' : err.message;
      set({ authError: msg });
      toast.error(msg);
      return { success: false, error: msg };
    }
  ...
  async verifyTouchId() {
    ...
    if (window.require && activeUser.webauthn_cred_id === 'electron-native-touch-id') {
      try {
        const { ipcRenderer } = window.require('electron');
        await ipcRenderer.invoke('prompt-touch-id', 'Authenticate to Gordan Belfort');
  ...
  async setPin(pin) {
    ...
    const updatedUsers = users.map(u => u.id === activeUser.id ? { ...u, pin_hash: hash } : u);
    const updatedUser = updatedUsers.find(u => u.id === activeUser.id);
  ...
  logout() { set({ isAuthenticated: false, authMode: 'picker', activeUser: null, authError: null }); },
  ...
  async toggleFamilyMode() {
    ...
    let holdings = nextMode ? await getFamilyHoldings() : await getHoldingsForUser(activeUser.id);
    
    holdings = holdings.map(h => ({
      ...h,
      _user: users.find(u => u.id === h.user_id) || { name: 'Unknown', color: '#ccc', initials: '?' }
    }));
