  ...
  return (
    <header className="topbar">
      ...
      <div className="topbar-badge" style={{ marginLeft: 'auto', marginRight: '16px', display: 'flex', alignItems: 'center', gap: '8px' }}>
        <div className="market-status-dot" style={{ background: 'var(--accent-gold)' }}></div>
        <span style={{ color: 'var(--text-secondary)' }}>Synced: {lastSynced}</span>
        <button
          onClick={() => initApp()}
          style={{ background: 'none', border: '1px solid var(--border-light)', borderRadius: '6px', cursor: 'pointer', padding: '4px', display: 'flex', alignItems: 'center', color: 'var(--text-secondary)', marginLeft: '4px' }}
          title="Force Sync"
        >
          <RefreshCw size={12} />
        </button>
      ...
      <div className="topbar-badge" style={{ display: 'flex', alignItems: 'center', gap: '6px', marginRight: '16px' }}>
        <span className="market-status-dot" style={{ background: 'var(--accent-green)', boxShadow: '0 0 8px var(--accent-green)' }} />
        <span style={{ color: 'var(--accent-green)', fontWeight: 500 }}>NSE Live</span>
      </div>

      <div className="forex-rate" title="Live USD/INR Rate">
        {usdInr ? fmtRate : <span style={{ color: 'var(--text-muted)' }}>Fetching rate...</span>}
      </div>
