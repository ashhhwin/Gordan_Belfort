...
import {
  ...
  BarChart2, Star, Bell, Newspaper, LogOut, Settings as SettingsIcon, Sparkles, Database
} from 'lucide-react';
    ...
    <aside className="sidebar">
      ...
      <div className="sidebar-bottom">
        <div className="sidebar-avatar">
          <div className="avatar-circle" style={{ background: activeUser?.color || 'linear-gradient(135deg, var(--accent-blue), var(--accent-purple))' }}>
            {activeUser?.initials || '??'}
          </div>
          <div className="avatar-info">
            <div className="avatar-name">{activeUser?.name || 'User'}</div>
            <div className="avatar-role">{activeUser?.role === 'admin' ? 'Administrator' : 'Family Member'}</div>
          </div>
        </div>
        <button
          onClick={logout}
          ...
          title="Lock"
        >
          <span className="sidebar-item-icon"><LogOut size={17} strokeWidth={1.8} /></span>
