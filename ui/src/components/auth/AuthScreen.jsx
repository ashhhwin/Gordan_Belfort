...
function TouchIdButton({ status, onClick, disabled }) {
  ...
  const ringClass = { idle: 'idle', scanning: 'scanning', success: 'success', error: 'error' }[status] ?? '';
  ...
  return (
    ...
    >
      <Fingerprint size={56} color={color} strokeWidth={1.2} style={{ transition: 'color 0.4s ease' }} />
    </button>
  );
}
...
export default function AuthScreen() {
  const {
    ...
    users, activeUser, selectUser, usdInr,
    cronStatus, syncStatus, earnings, totalDayGain
  } = useStore();
    ...
    async function fetchWeather() {
      try {
        ...
        if (ipData.latitude && ipData.longitude) {
           const wRes = await fetch(`https://api.open-meteo.com/v1/forecast?latitude=${ipData.latitude}&longitude=${ipData.longitude}&current_weather=true&temperature_unit=celsius`);
  ...
  useEffect(() => {
    async function fetchTicker() {
      ...
      const symbols = ['SPY', 'QQQ', '^IXIC', '^RUT', '^NSEI', '^BSESN', '^VIX', 'GC=F', 'CL=F', 'BTC-USD', 'ETH-USD', 'INR=X'];
      const labels = {
        'SPY': 'SPY',
        'QQQ': 'QQQ',
        '^IXIC': 'NASDAQ',
      ...
      try {
        const results = await Promise.all(symbols.map(async (sym) => {
          try {
            const res = await fetch(`/api/yahoo/v8/finance/chart/${sym}?interval=1d&range=2d`);
        ...
        const usSectorSymbols = ['XLK', 'XLV', 'XLF', 'XLY', 'XLC', 'XLI', 'XLP', 'XLE', 'XLU', 'XLRE', 'XLB'];
        const usSectorLabels = {
          'XLK': 'TEC', 'XLV': 'HEA', 'XLF': 'FIN', 'XLY': 'CND',
          'XLC': 'COM', 'XLI': 'IND', 'XLP': 'STP', 'XLE': 'ENE',
          'XLU': 'UTL', 'XLRE': 'REL', 'XLB': 'MAT'
        };
        const usResults = await Promise.all(usSectorSymbols.map(async (sym) => {
          try {
            const res = await fetch(`/api/yahoo/v8/finance/chart/${sym}?interval=1d&range=2d`);
            const data = await res.json();
            const result = data?.chart?.result?.[0];
        ...
        const inSectorSymbols = ['^CNXIT', '^CNXAUTO', '^CNXFIN', '^CNXFMCG', '^CNXMETAL', '^CNXPHARMA', '^CNXPSUBANK', '^CNXREALTY', '^CNXENERGY', '^CNXINFRA', '^CNXMEDIA'];
        const inSectorLabels = {
          '^CNXIT': 'IT', '^CNXAUTO': 'AUT', '^CNXFIN': 'FIN', '^CNXFMCG': 'FMG',
          '^CNXMETAL': 'MET', '^CNXPHARMA': 'PHA', '^CNXPSUBANK': 'PSU', '^CNXREALTY': 'REL',
          '^CNXENERGY': 'ENE', '^CNXINFRA': 'INF', '^CNXMEDIA': 'MED'
        };
        const inResults = await Promise.all(inSectorSymbols.map(async (sym) => {
          try {
            const res = await fetch(`/api/yahoo/v8/finance/chart/${sym}?interval=1d&range=2d`);
            const data = await res.json();
            const result = data?.chart?.result?.[0];
  ...
  useEffect(() => {
    if (authMode === 'touchid') {
      ...
      setHint(isFirstRun ? 'Press fingerprint to enroll' : 'Touch fingerprint sensor to unlock');
    } else if (authMode === 'pin') {
      setHint(isFirstRun ? (pinStep === 'confirm' ? 'Confirm your PIN' : 'Set a 6-digit PIN') : 'Enter Passcode');
    }
  ...
  const renderTickerItems = () => {
    ...
    return tickerData.map((t, idx) => (
      <div key={idx} className="ticker-item">
        <span className={t.pct >= 0 ? 'ticker-up' : 'ticker-down'}>{t.pct >= 0 ? '▲' : '▼'}</span>
        {' '}
        {t.label} {t.price < 100 ? t.price.toFixed(2) : t.price.toLocaleString(undefined, { maximumFractionDigits: 2 })}
        {' '}
        <span style={{ color: t.pct >= 0 ? 'var(--accent-green)' : 'var(--accent-red)', marginLeft: 4 }}>
  ...
  return (
    <div className="auth-bg">
      ...
      <div className="lock-status">
        ...
        <div className="lock-status-item">
          SYNC {syncStatus === 'syncing' ? 'RUNNING' : syncStatus === 'error' ? 'WARNING' : 'HEALTHY'} 
          <div className={`status-dot ${syncStatus === 'error' ? 'red' : syncStatus === 'syncing' ? 'yellow' : 'green'}`} />
        </div>
      </div>
      ...
      <div className="market-events">
        ...
        {earnings && earnings.slice(0, 3).map((event, idx) => (
          <div key={idx} className="event-card">
            <div className="event-title">{event.symbol || 'Unknown'}</div>
            <div className="event-desc">{event.company_name || 'Market Event'}</div>
          </div>
        ))}
      ...
      <div className="sector-heatmap-wrapper">
        <div className="sector-heatmap">
          ...
          <div className="heatmap-grid">
            {sectorData.length > 0 ? sectorData.map((sec, idx) => {
              const color = sec.pct >= 0 ? `rgba(0, 212, 161, ${Math.min(0.2 + (sec.pct / 2), 1)})` : `rgba(255, 77, 106, ${Math.min(0.2 + (Math.abs(sec.pct) / 2), 1)})`;
              return (
                <div 
                  key={idx} 
                ...
                >
                  ...
                  <span className="heatmap-val">{sec.pct > 0 ? '+' : ''}{sec.pct.toFixed(1)}%</span>
                </div>
              );
            }) : (
              [...Array(11)].map((_, i) => <div key={i} className="heatmap-square" style={{ backgroundColor: 'var(--surface-light)' }} />)
            )}
          </div>
        </div>
        ...
        <div className="sector-heatmap">
          <h4>IND Sectors</h4>
          <div className="heatmap-grid">
            {inSectorData.length > 0 ? inSectorData.map((sec, idx) => {
              const color = sec.pct >= 0 ? `rgba(0, 212, 161, ${Math.min(0.2 + (sec.pct / 2), 1)})` : `rgba(255, 77, 106, ${Math.min(0.2 + (Math.abs(sec.pct) / 2), 1)})`;
              return (
                <div 
                  key={idx} 
      ...
      <div className="auth-center">
        ...
        <div className="glimpse-badge glimpse-expanded">
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            ...
            <span style={{ color: (totalDayGain || 0) >= 0 ? 'var(--accent-green)' : 'var(--accent-red)', fontWeight: 600 }}>
              {(totalDayGain || 0) >= 0 ? '+' : '-'}${Math.abs(totalDayGain || 0).toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2})}
            </span>
          </div>
          <div className="glimpse-separator" />
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            <span style={{ color: 'var(--text-muted)' }}>USA (SPY):</span>
            <span style={{ color: spyPct >= 0 ? 'var(--accent-green)' : 'var(--accent-red)', fontWeight: 600 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            ...
            <span style={{ color: nseiPct >= 0 ? 'var(--accent-green)' : 'var(--accent-red)', fontWeight: 600 }}>
              {nseiPct >= 0 ? '+' : ''}{nseiPct.toFixed(2)}%
            </span>
          </div>
        </div>
        ...
        {authMode === 'touchid' && (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', minHeight: 200, zIndex: 10 }}>
            ...
            <button onClick={() => setAuthMode('pin')} className="auth-switch-btn" style={{marginTop: 32}}>
        ...
        {authMode === 'pin' && (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', minHeight: 300, zIndex: 10 }}>
            <p className="auth-touch-hint" style={{ marginTop: 0, marginBottom: 24, color: authError ? 'var(--accent-red)' : 'var(--text-muted)' }}>
              {hint}
            </p>
